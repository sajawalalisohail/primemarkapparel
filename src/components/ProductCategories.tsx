        ...
        >
          {products.map((product, index) => (
            <motion.div
              key={index}
              variants={cardReveal}
              className="group rounded-xl bg-white border border-slate-200 shadow-sm cursor-default transition-all duration-300 ease-out hover:shadow-lg hover:border-blue-400/30 overflow-hidden"
              whileHover={{ y: -6, scale: 1.02 }}
            >
              {/* Placeholder image area */}
              <div className={`w-full h-40 ${product.placeholderColor} flex items-center justify-center`}>
                <div className="opacity-40 transition-transform duration-300 ease-out group-hover:scale-110">
                  {product.icon}
                </div>
              </div>
