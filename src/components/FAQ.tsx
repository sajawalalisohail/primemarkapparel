  ...
  return (
    <div className="border-b border-slate-200 last:border-b-0">
      <button
        type="button"
        onClick={onToggle}
        className="w-full flex items-center justify-between py-4 text-left hover:text-blue-900 transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-white rounded-lg"
        aria-expanded={isOpen}
      >
        <span className="text-slate-900 font-medium pr-4 text-sm sm:text-base">{question}</span>
...
export default function FAQ({ hideHeader = false }: FAQProps) {
  ...
  const handleFAQToggle = (categoryTitle: string, index: number) => {
    // Close if clicking the same FAQ, otherwise open the new one
    if (openIndex?.category === categoryTitle && openIndex?.index === index) {
      setOpenIndex(null);
    } else {
      setOpenIndex({ category: categoryTitle, index });
    }
  };
        ...
        <div className="grid md:grid-cols-2 gap-6 items-start">
          {faqCategories.map((category) => {
            ...
            return (
              <div
                key={category.title}
                className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden self-start"
              >
                {/* Category Header */}
                <button
                  type="button"
                ...
                {isCategoryOpen ? (
                  <div className="px-5 py-2">
                    {category.faqs.map((faq, index) => {
                      const uniqueKey = `${category.title}-${index}`;
                      const isOpen =
                        openIndex?.category === category.title &&
                      ...
                      return (
                        <FAQItem
                          ...
                          answer={faq.answer}
                          isOpen={isOpen}
                          onToggle={() => handleFAQToggle(category.title, index)}
                        />
                      );
                    })}
                  </div>
